...
export default function App() {
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await fetch("http://192.168.1.12:5000/api/users/register", {
        method: "POST",
  ...
  const handleLogin = async () => {
    try {
      const response = await fetch("http://192.168.1.12:5000/api/users/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (response.ok) {
        ...
        if (token) {
          ...
          await fetch("http://192.168.1.12:5000/api/users/save-push-token", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
