...
export default function WalletScreen({ provider_id }) {
    ...
    const loadWallet = async () => {
        try {
            ...
            const res = await axios.get(`http://${ip}:5000/api/provider/wallet/${id}`);
            setWallet({
    ...
    const payDebt = async () => {
        ...
        if (numAmount > wallet.debt) return Alert.alert("Error", "Amount exceeds debt");
        if (numAmount > wallet.balance) return Alert.alert("Error", "Insufficient balance");
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "center",marginBottom:20 }}>
                <Ionicons name="wallet" size={28} color="#6c3483" style={{ marginRight: 8 }} />
